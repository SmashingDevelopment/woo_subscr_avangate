...
if ( ! function_exists( 'getAvangateSettings' ) ) {
	function getAvangateSettings()
	{
		global $woocommerce;
		...
		foreach ($woocommerce->payment_gateways->payment_gateways as $key => $gateway)
		{
			...
			if ($woocommerce->payment_gateways->payment_gateways[$key]->id !== Gateway_Avangate::GATEWAY_ID)
				continue;

			$avangate_settings = $gateway->settings;
			break;
		}
		return $avangate_settings;
	}
}
