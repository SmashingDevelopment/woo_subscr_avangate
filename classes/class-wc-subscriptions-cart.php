...
class WC_Subscriptions_Cart {
	...
	public static function get_cart_item_from_session( $session_data, $values, $key ) {
		...
		if ( isset( $values['subscription_renewal'] ) ) {
			...
			$title = sprintf( __( 'Renewal of "%s"', WC_Subscriptions::$text_domain ), $_product->post->post_title );
	...
	/**
	 * For subscription renewal via cart, adjust item price by original order discount
	 *
	 * Original order discounts are set in @see self::before_calculate_totals() so that calculations are correct in
	 * @see WC_Cart::calculate_totals() and @see self::calculate_subscription_totals(). However during the calculations,
	 * the price of the item is referenced several times. That price also needs to be adjusted to reflect any original order
	 * discounts which is what this function does.
	 *
	 * @since 1.3
	 */	
	public static function get_discounted_price_for_renewal( $price, $values, $cart ) {
		...
		if ( $cart_item ) {
			...
			$price -= WC_Subscriptions_Order::get_meta( $original_order_id, '_order_recurring_discount_total', 0 );
		}
