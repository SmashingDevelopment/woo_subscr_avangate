...
class WC_Subscriptions_Admin {
	...
	public static function init() {
		...
		add_filter( 'manage_woocommerce_page_subscriptions_columns', __CLASS__ . '::get_subscription_table_columns' );
	}
...
?>
	...
	<td>
		<label><?php printf( __( 'Sign-up Fee (%s)', WC_Subscriptions::$text_domain ), get_woocommerce_currency_symbol() ) ?></label>
<?php
		// Sign-up Fee
		woocommerce_wp_text_input( array(
...
<?php
		...
		woocommerce_wp_text_input( array(
			...
			'label'         => __( 'Free Trial', WC_Subscriptions::$text_domain ),
			'placeholder'   => __( 'e.g. 1', WC_Subscriptions::$text_domain ),
...
<?php
	...
	public static function variable_subscription_bulk_edit_actions() { ?>
		<option value="_subscription_sign_up_fee"><?php _e( 'Subscription sign-up fee', WC_Subscriptions::$text_domain ); ?></option>
		<option value="_subscription_period_interval" rel="select"><?php _e( 'Subscription billing interval', WC_Subscriptions::$text_domain ); ?></option>
		<option value="_subscription_period" rel="select"><?php _e( 'Subscription period', WC_Subscriptions::$text_domain ); ?></option>
		<option value="_subscription_length" rel="select"><?php _e( 'Subscription length', WC_Subscriptions::$text_domain ); ?></option>
		<option value="_subscription_trial_length"><?php _e( 'Free trial length', WC_Subscriptions::$text_domain ); ?></option>
		<option value="_subscription_trial_period" rel="select"><?php _e( 'Free trial period', WC_Subscriptions::$text_domain ); ?></option>
<?php
	}
	...
	public static function save_subscription_meta( $post_id ) {
		...
		if ( ! WC_Subscriptions_Product::is_subscription( $post_id ) || ( isset( $_REQUEST['_subscription_price'] ) && is_array( $_REQUEST['_subscription_price'] ) ) )
			return;
	...
	public static function process_product_meta_variable_subscription( $post_id ) {
		...
		$min_variation_price = get_post_meta( $post_id, '_min_variation_price', true );

		$parent_billing_period = '';
		...
		for ( $i = 0; $i <= $max_loop; $i ++ ) {
			...
			if ( isset( $_POST['_subscription_price'] ) && is_array( $_POST['_subscription_price'] ) ) {

				$subscription_price = woocommerce_clean( $_POST['_subscription_price'][$i] );
				...
				// Update price if on sale
				if ( ! empty( $_POST['variable_sale_price'][$i] ) && ( ( empty( $date_to ) && empty( $date_from ) ) || ( $date_from < $now && ( empty( $date_to ) || $date_to > $now ) ) ) )
					$price = woocommerce_clean( $_POST['variable_sale_price'][$i] );
				else
			...
			$max_trial_length = count( $subscription_ranges[$_POST['_subscription_trial_period'][$i]] ) - 1;

			$_POST['_subscription_trial_length'][$i] = absint( $_POST['_subscription_trial_length'][$i] );

			if ( $_POST['_subscription_trial_length'][$i] > $max_trial_length )
				$_POST['_subscription_trial_length'][$i] = $max_trial_length;
			...
			foreach ( $subscription_fields as $field_name )
				if ( isset( $_POST[$field_name][$i] ) )
					update_post_meta( $variation_id, $field_name, woocommerce_clean( $_POST[$field_name][$i] ) );

			// Set the default billing periods on the subscription
			if ( $price === $min_variation_price ) {
				$parent_billing_period = WC_Subscriptions::get_longest_period( $parent_billing_period, $_POST['_subscription_period'][$i] );
				update_post_meta( $post_id, '_subscription_period', woocommerce_clean( $_POST['_subscription_period'][$i] ) );
			}
		}
