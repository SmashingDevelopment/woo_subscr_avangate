...
class Subscription_Order {
	...
	public static function get_cart_discount_to_display( $discount, $order ) {
		...
		if( self::order_contains_subscription( $order ) ) {
			...
			$discount = sprintf( __( '%s discount','plugin' ), $discount );
		}
	...
	public static function subscription_thank_you( $order_id ){
		...
		if( self::order_contains_subscription( $order_id ) ) {
			$thank_you_message = '<p>' . __( 'Your subscription will be activated when payment clears.','plugin' ) . '</p>';
			$thank_you_message = sprintf( __( '%sView the status of your subscription in %syour account%s.%s','plugin' ), '<p>', '<a href="' . get_permalink( woocommerce_get_page_id( 'myaccount' ) ) . '">', '</a>','</p>' );
	...
	public static function prefill_order_item_meta_old() {
		...
		check_ajax_referer('plugin', 'security' );
...
?>
	<p id="recurring_shipping_description"<?php if ( ! $contains_subscription ) echo $display_none; ?>>
		<?php _e( 'Shipping cost is included in the recurring total.','plugin' ); ?>
	</p>
	<div class="clear"></div>
</div>
<div id="gateway_support"<?php if ( ! $contains_subscription ) echo $display_none; ?>>
	...
	<div class="error"<?php if ( ! $contains_subscription || $changes_supported == 'true' ) echo $display_none; ?>>
		<p><?php printf( __( 'The %s payment gateway is used to charge automatic subscription payments for this order. This gateway <strong>does not</strong> support changing a subscription\'s details.','plugin' ), get_post_meta( $post_id, '_payment_method_title', true ) ); ?></p>
		<p>
			<?php _e( 'It is strongly recommended you <strong>do not change</strong> any of the recurring totals or subscription item\'s details.','plugin' ); ?>
			<a href="http://docs.woothemes.com/document/add-or-modify-a-subscription/#manuallymodifyinganexistingsubscription"><?php _e( 'Learn More','plugin' ); ?> &raquo;</a>
		</p>
	</div>
</div>
<div id="recurring_order_totals"<?php if ( ! $contains_subscription ) echo $display_none; ?>>
	<h3><?php _e( 'Recurring Totals','plugin'); ?></h3>
	<div class="totals_group">
		<h4><?php _e( 'Recurring Discounts','plugin'); ?> <a class="tips" data-tip="<?php _e( 'The discounts applied to each recurring payment charged in the future.','plugin' ); ?>" href="#">[?]</a></h4>
		<ul class="totals">

			<li class="left">
				<label><?php _e( 'Cart Discount:','plugin' ); ?></label>
			...
			<li class="right">
				<label><?php _e( 'Order Discount:','plugin' ); ?></label>
	...
	<div class="totals_group tax_rows_group">
		<h4><?php _e( 'Recurring Tax Rows','plugin' ); ?> <a class="tips" data-tip="<?php _e( 'These rows contain taxes included in each recurring amount for this subscription. This allows you to display multiple or compound taxes rather than a single total on future subscription renewal orders.','plugin' ); ?>" href="#">[?]</a></h4>
			...
			<?php
				...
				if ( is_array( $taxes ) && sizeof( $taxes ) > 0 ) :
					...
					foreach ( $taxes as $item_id => $item ) : ?>
						...
						<div class="tax_row">
							<p class="first">
								<label><?php _e( 'Recurring Tax Label:','plugin' ); ?></label>
		...
		<h4 style="padding-bottom: 10px;"><a href="#" class="add_recurring_tax_row"><?php _e( '+ Add tax row','plugin' ); ?></a></h4>
		<div class="clear"></div>
	</div>
	<div class="totals_group">
		<h4><?php _e( 'Recurring Totals','plugin' ); ?> <a class="tips" data-tip="<?php _e( 'The total amounts charged for each future recurring payment.','plugin' ); ?>" href="#">[?]</a></h4>
		<ul class="totals">

			<li class="left">
				<label><?php _e( 'Recurring Tax Total:','plugin' ); ?></label>
			...
			<li class="right">
				<label><?php _e( 'Recurring Order Total:','plugin' ); ?></label>
