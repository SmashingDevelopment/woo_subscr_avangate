...
class Subscription_Manager {
	...
	public static function activate_subscription( $user_id, $subscription_key ) {
		...
		$item = Subscription_Order::get_item_by_product_id( $order, $subscription['product_id'] );

		if ( $subscription['status'] != 'pending' && ! self::can_subscription_be_changed_to( 'active', $subscription_key, $user_id ) ) {

		 	$order->add_order_note( sprintf( __( 'Unable to activate subscription "%s".','plugin' ), $item['name'] ) );
			...
			$order->add_order_note( sprintf( __( 'Activated Subscription "%s".','plugin' ), $item['name'] ) );
	...
	public static function put_subscription_on_hold( $user_id, $subscription_key ) {
		...
		if ( ! self::can_subscription_be_changed_to( 'on-hold', $subscription_key, $user_id ) ) {
		 	...
		 	$order->add_order_note( sprintf( __( 'Unable to put subscription on-hold: "%s".','plugin' ), $item['name'] ) );
			...
			$order->add_order_note( sprintf( __( 'Subscription On-hold: "%s".','plugin' ), $item['name'] ) );
	...
	public static function cancel_subscription( $user_id, $subscription_key ) {
		...
		if ( ! self::can_subscription_be_changed_to( 'cancelled', $subscription_key, $user_id ) ) {
		 	...
		 	$order->add_order_note( sprintf( __( 'Unable to cancel subscription "%s".','plugin' ), $item['name'] ) );
			...
			// Log cancellation on order
			$order->add_order_note( sprintf( __( 'Cancelled Subscription "%s".','plugin' ), $item['name'] ) );
	...
	public static function failed_subscription_signup( $user_id, $subscription_key ) {
		...
		$order->add_order_note( sprintf( __( 'Failed sign-up for subscription "%s".','plugin' ), $item['name'] ) );
	...
	public static function trash_subscription( $user_id, $subscription_key ) {
		...
		if ( ! self::can_subscription_be_changed_to( 'trash', $subscription_key, $user_id ) ) {
		 	...
		 	$order->add_order_note( sprintf( __( 'Unable to trash subscription "%s".','plugin' ), $item['name'] ) );
			...
			$order->add_order_note( sprintf( __( 'Trashed Subscription "%s".','plugin' ), $item['name'] ) );
	...
	public static function delete_subscription( $user_id, $subscription_key ) {
		...
		if ( ! self::can_subscription_be_changed_to( 'deleted', $subscription_key, $user_id ) ) {
			...
			// Log deletion on order
			$order->add_order_note( sprintf( __( 'Deleted Subscription "%s".','plugin' ), $item['name'] ) );
	...
	public static function expire_subscription( $user_id, $subscription_key ) {
		...
		$order->add_order_note( sprintf( __( 'Subscription Expired: "%s".','plugin' ), $item['name'] ) );
	...
	public static function process_subscription_payment( $user_id, $subscription_key ) {
		...
		$order->add_order_note( sprintf( __( 'Payment received for subscription "%s"','plugin' ), $item['name'] ) );
	...
	public static function process_subscription_payment_failure( $user_id, $subscription_key ) {
		...
		if ( $failed_payments_exceeded || ( $subscription['failed_payments'] >= $max_failed_payments && intval( $max_failed_payments ) !== 0 ) ) {
			...
			$order->add_order_note( sprintf( __( 'Cancelled Subscription "%s". Maximum number of failed subscription payments reached.','plugin' ), $item['name'] ) );
			...
			$order->add_order_note( sprintf( __( 'Payment failed for subscription "%s".','plugin' ), $item['name'] ) );
	...
	public static function process_subscription_payments_on_order( $order, $product_id = '' ) {
		...
		if ( empty( $product_id ) ) {
			$order_items      = Subscription_Order::get_recurring_items( $order );
			$first_order_item = reset( $order_items );
			$product_id       = Subscription_Order::get_items_product_id( $first_order_item );
		}

		if ( Subscription_Order::order_contains_subscription( $order ) && Subscription_Order::is_item_subscription( $order, $product_id ) ) {
	...
	public static function activate_subscriptions_for_order( $order ) {
		...
		if ( ! Subscription_Order::order_contains_subscription( $order ) )
			return;
	...
	public static function failed_subscription_sign_ups_for_order( $order ) {
		...
		if ( $order->status != 'failed' )
			$order->update_status( 'failed', __( 'Subscription sign up failed.','plugin' ) );
	...
	public static function create_pending_subscription_for_order( $order, $product_id, $args = array() ) {
		...
		$order->add_order_note( sprintf( __( 'Pending subscription created for "%s".','plugin' ), $product->get_title() ) );
	...
	public static function update_users_subscriptions_for_order( $order, $status = 'pending' ) {
		...
		foreach( $order->get_items() as $order_item ) {
			...
			if ( ! Subscription_Order::is_item_subscription( $order, $order_item ) )
				continue;

			$subscription_key = self::get_subscription_key( $order->id, Subscription_Order::get_items_product_id( $order_item ) );
			...
			switch ( $status ) {
				...
				case 'pending' :
				default :
					self::create_pending_subscription_for_order( $order, Subscription_Order::get_items_product_id( $order_item ) );
					break;
			}
		}
	...
	public static function maybe_trash_subscription( $order ) {
		...
		if ( Subscription_Order::order_contains_subscription( $order ) ) {
			foreach( $order->get_items() as $order_item ) {
				if ( Subscription_Order::is_item_subscription( $order, $order_item ) ) {
					self::trash_subscription( $order->customer_user, self::get_subscription_key( $order->id, Subscription_Order::get_items_product_id( $order_item ) ) );
				}
			}
		}
	...
	public static function can_subscription_be_changed_to( $new_status, $subscription_key, $user_id = '' ) {
		...
		if ( empty( $subscription ) ) {
			...
			$order_uses_manual_payments = ( empty( $payment_gateway ) || Subscription_Order::requires_manual_renewal( $order ) ) ? true : false;
	...
	public static function get_status_to_display( $status, $subscription_key = '', $user_id = 0 ) {
		...
		switch ( $status ) {
			case 'active' : 
				$status_string = __( 'Active','plugin' );
				break;
			case 'cancelled' :
				$status_string = __( 'Cancelled','plugin' );
				break;
			case 'expired' :
				$status_string = __( 'Expired','plugin' );
				break;
			case 'pending' :
				$status_string = __( 'Pending','plugin' );
				break;
			case 'failed' :
				$status_string = __( 'Failed','plugin' );
				break;
			case 'on-hold' :
			case 'suspend' : // Backward compatibility
				$status_string = __( 'On-hold','plugin' );
				break;
			default :
	...
	public static function get_subscription_period_strings( $number = 1, $period = '' ) {
		...
		$translated_periods = apply_filters( 'woocommerce_subscription_periods',
			array(
				'day'   => sprintf( _n( 'day', '%s days', $number,'plugin' ), $number ),
				'week'  => sprintf( _n( 'week', '%s weeks', $number,'plugin' ), $number ),
				'month' => sprintf( _n( 'month', '%s months', $number,'plugin' ), $number ),
				'year'  => sprintf( _n( 'year', '%s years', $number,'plugin' ), $number )
			)
		);
	...
	public static function get_subscription_period_interval_strings( $interval = '' ) {
		...
		$intervals = array( 1 => __( 'per','plugin' ) );

		foreach ( range( 2, 6 ) as $i )
			$intervals[$i] = sprintf( __( 'every %s','plugin' ), WC_Subscriptions::append_numeral_suffix( $i )  );
	...
	public static function get_subscription_ranges( $subscription_period = '' ) {
		...
		foreach ( array( 'day', 'week', 'month', 'year' ) as $period ) {
			...
			$subscription_lengths = array( 
				__( 'all time','plugin' ),
				sprintf( __( '%s %s','plugin' ), 1, $subscription_periods[$period] ),
			);
	...
	public static function get_subscription_trial_lengths( $subscription_period = '' ) {
		...
		foreach ( $all_trial_periods as $period => $trial_periods )
			$all_trial_periods[$period][0] = __( 'no','plugin' ); // "No Trial Period"
	...
	public static function get_subscription_trial_period_strings( $number = 1, $period = '' ) {
		...
		$translated_periods = apply_filters( 'woocommerce_subscription_trial_periods',
			array(
				'day'   => sprintf( _n( '%s day', 'a %s-day', $number,'plugin' ), $number ),
				'week'  => sprintf( _n( '%s week', 'a %s-week', $number,'plugin' ), $number ),
				'month' => sprintf( _n( '%s month', 'a %s-month', $number,'plugin' ), $number ),
				'year'  => sprintf( _n( '%s year', 'a %s-year', $number,'plugin' ), $number )
			)
		);
	...
	public static function get_available_time_periods( $form = 'singular' ) {
		...
		$translated_periods = apply_filters( 'woocommerce_subscription_available_time_periods',
			array(
				'day'   => _n( 'day', 'days', $number,'plugin' ),
				'week'  => _n( 'week', 'weeks', $number,'plugin' ),
				'month' => _n( 'month', 'months', $number,'plugin' ),
				'year'  => _n( 'year', 'years', $number,'plugin' )
			)
		);
	...
	public static function calculate_subscription_expiration_date( $subscription_key, $user_id = '', $type = 'mysql' ) {
		...
		if ( empty( $subscription ) ) {
			...
			$subscription_period       = Subscription_Order::get_subscription_period( $order, $subscription['product_id'] );
			$subscription_length       = Subscription_Order::get_subscription_length( $order, $subscription['product_id'] );
			$subscription_interval     = Subscription_Order::get_subscription_interval( $order, $subscription['product_id'] );
			$subscription_trial_length = Subscription_Order::get_subscription_trial_length( $order, $subscription['product_id'] );
			$subscription_trial_period = Subscription_Order::get_subscription_trial_period( $order, $subscription['product_id'] );
	...
	public static function calculate_next_payment_date( $subscription_key, $user_id = '', $type = 'mysql', $from_date = '' ) {
		...
		$next_payment = Subscription_Order::calculate_next_payment_date( $subscription['order_id'], $subscription['product_id'], $type, $from_date );
	...
	public static function requires_manual_renewal( $subscription_key, $user_id = '' ) {
		...
		if ( isset( $subscription['order_id'] ) )
			$requires_manual_renewals = Subscription_Order::requires_manual_renewal( $subscription['order_id'] );
		else
	 ...
	 * @return bool True if the user has the subscription (or any subscription if no subscription specified), otherwise false.
	 */
	public static function user_has_subscription( $user_id = 0, $product_id = '' ) {
	...
	public static function maybe_change_users_subscription() {
		...
		if ( isset( $_GET['change_subscription_to'] ) && isset( $_GET['subscription_key'] ) && isset( $_GET['_wpnonce'] )  ) {
			...
			if ( wp_verify_nonce( $_GET['_wpnonce'], $_GET['subscription_key'] ) === false ) {
				...
				$woocommerce->add_error( sprintf( __( 'That subscription can not be changed to %s. Please contact us if you need assistance.','plugin' ), $_GET['change_subscription_to'] ) );

			} elseif ( empty( $subscription ) ) {

				$woocommerce->add_error( __( 'That doesn\'t appear to be one of your subscriptions.','plugin' ) );
				...
				$woocommerce->add_error( sprintf( __( 'Unknown subscription status: "%s". Please contact us if you need assistance.','plugin' ), $_GET['change_subscription_to'] ) );
				...
				switch ( $_GET['change_subscription_to'] ) {
					case 'active' :
						if ( WC_Subscriptions_Manager::subscription_requires_payment( $_GET['subscription_key'], $user_id ) )
							$woocommerce->add_error( sprintf( __( 'You can not reactive that subscription until paying to renew it. Please contact us if you need assistance.','plugin' ), $_GET['change_subscription_to'] ) );
						else {
							self::reactivate_subscription( $user_id, $_GET['subscription_key'] );
							$status_message = __( 'reactivated','plugin' );
						}
						break;
					case 'on-hold' :
						if ( self::current_user_can_suspend_subscription( $_GET['subscription_key'] ) ) {
							...
							$status_message = __( 'suspended','plugin' );
						} else {
							$woocommerce->add_error( sprintf( __( 'You can not suspend that subscription - the suspension limit has been reached. Please contact us if you need assistance.','plugin' ), $_GET['change_subscription_to'] ) );
						}
						break;
					case 'cancelled' :
						self::cancel_subscription( $user_id, $_GET['subscription_key'] );
						$status_message = __( 'cancelled','plugin' );
						break;
				}
				...
				if ( isset( $status_message ) ) {
					...
					$order->add_order_note( sprintf( __( 'The status of subscription %s was changed to %s by the subscriber from their account page.','plugin' ), $_GET['subscription_key'], $_GET['change_subscription_to'] ) );

					$woocommerce->add_message( sprintf( __( 'Your subscription has been %s.','plugin' ), $status_message ) );
				}
			}
	...
	public static function ajax_update_next_payment_date() {
		...
		if ( ! wp_verify_nonce( $_POST['wcs_nonce'],'plugin' ) ) {

			$response['message'] = sprintf( '<div class="error">%s</div>', __( 'Invalid security token, please reload the page and try again.','plugin' ) );

		} elseif ( ! current_user_can( 'manage_woocommerce' ) ) {

			$response['message'] = sprintf( '<div class="error">%s</div>', __( 'Only store managers can edit payment dates.','plugin' ) );

		} elseif ( empty( $_POST['wcs_day'] ) || empty( $_POST['wcs_month'] ) || empty( $_POST['wcs_year'] ) ) {

			$response['message'] = sprintf( '<div class="error">%s</div>', __( 'Please enter all date fields.','plugin' ) );
			...
			if ( is_wp_error( $new_payment_timestamp ) ) {
				...
				if ( $time_diff > 0 && $time_diff < 7 * 24 * 60 * 60 )
					$date_to_display = sprintf( __( 'In %s','plugin' ), human_time_diff( time(), $new_payment_timestamp ) );
				else
					$date_to_display = date_i18n( get_option( 'date_format' ), $new_payment_timestamp_user_time );

				$response['status']        = 'success';
				$response['message']       = sprintf( '<div class="updated">%s</div>', __( 'Date Changed','plugin' ) );
	...
	public static function update_next_payment_date( $new_payment_date, $subscription_key, $user_id = '', $timezone = 'server' ) {
		...
		if ( 0 == $next_payment_timestamp ) {
			...
			$response = new WP_Error( 'invalid-date', __( 'This subscription does not have any future payments.','plugin' ) );

		} elseif ( $new_payment_timestamp <= time() ) {

			$response = new WP_Error( 'invalid-date', __( 'Please enter a date in the future.','plugin' ) );

		} elseif ( 0 != $trial_expiration_timestamp && $new_payment_timestamp <= $trial_expiration_timestamp ) {

			$response = new WP_Error( 'invalid-date', sprintf( __( 'Please enter a date after the trial period ends (%s).','plugin' ), date_i18n( get_option( 'date_format' ), $trial_expiration_timestamp + ( get_option( 'gmt_offset' ) * 3600 ) ) ) );

		} elseif ( 0 != $expiration_timestamp && $new_payment_timestamp >= $expiration_timestamp ) {

			$response = new WP_Error( 'invalid-date', __( 'Please enter a date before the expiration date.','plugin' ) );
			...
			$response = new WP_Error( 'misc-error', __( 'The payment date could not be changed.','plugin' ) );
	...
	public static function safeguard_scheduled_payments( $user_id, $subscription_key ) {
		...
		if ( $payments_blocked_until > time() || in_array( $subscription['status'], array( 'suspended', 'on-hold', 'cancelled', 'expired', 'failed' ) ) ) {
	...
	public static function get_subscription_price_string( $subscription_details ) {
		...
		$subscription_details = wp_parse_args( $subscription_details, array(
				'initial_amount'        => '',
				'initial_description'   => __( 'up front','plugin' ),
				'recurring_amount'      => '',
		...
		if ( $subscription_details['subscription_length'] > 0 && $subscription_details['subscription_length'] == $subscription_details['subscription_interval'] )
			if ( ! empty( $subscription_details['initial_amount'] ) )
				$subscription_string = sprintf( __( '%s %s then %s','plugin' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string );
			else 
				$subscription_string = $recurring_amount_string;
		elseif ( ! empty( $subscription_details['initial_amount'] ) )
			$subscription_string = sprintf( _n( '%s %s then %s / %s', '%s %s then %s every %s', $subscription_details['subscription_interval'],'plugin' ), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, $subscription_period_string );
		elseif ( ! empty( $subscription_details['recurring_amount'] ) || intval( $subscription_details['recurring_amount'] ) === 0 )
			$subscription_string = sprintf( _n( '%s / %s', ' %s every %s', $subscription_details['subscription_interval'],'plugin' ), $recurring_amount_string, $subscription_period_string );
		else
			$subscription_string = '';

		if ( $subscription_details['subscription_length'] > 0 )
			$subscription_string = sprintf( __( '%s for %s','plugin' ), $subscription_string, $subscription_ranges[$subscription_details['subscription_period']][$subscription_details['subscription_length']] );
		...
		if ( $subscription_details['trial_length'] > 0 ) {
			...
			if ( ! empty( $subscription_details['initial_amount'] ) )
				$subscription_string = sprintf( __( '%s after %s free trial','plugin' ), $subscription_string, $trial_length );
			else
				$subscription_string = sprintf( __( '%s free trial then %s','plugin' ), ucfirst( $trial_length ), $subscription_string );
		}
	...
	public static function touch_time( $args = array() ) {
		...
		if ( $args['include_buttons'] ) {
			$touch_time .= '<p>';
			$touch_time .= '<a href="#edit_timestamp" class="save-timestamp hide-if-no-js button">' . __( 'Change','plugin' ) . '</a>';
			$touch_time .= '<a href="#edit_timestamp" class="cancel-timestamp hide-if-no-js">' . __( 'Cancel','plugin' ) . '</a>';
			$touch_time .= '</p>';
		}
