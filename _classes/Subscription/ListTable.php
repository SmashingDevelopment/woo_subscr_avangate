...
class Subscription_ListTable extends WP_List_Table
{
...
//
//				$all_statuses = array(
//					'active' => __('Reactivate', 'plugin'),
//					'on-hold' => __('Suspend', 'plugin'),
//					'cancelled' => __('Cancel', 'plugin'),
//					'trash' => __('Trash', 'plugin'),
//					'deleted' => __('Delete Permanently', 'plugin'),
//				);
//
//				foreach ($all_statuses as $status => $label)
//				{
//					if (Subscription_Manager::can_subscription_be_changed_to($status, $item['subscription_key'], $item['user_id']))
//					{
...
//
//				$column_content = sprintf('<mark class="%s">%s</mark> %s', sanitize_title($item[$column_name]), Subscription_Manager::get_status_to_display($item[$column_name], $item['subscription_key'], $item['user_id']), $this->row_actions($actions));
...
//				$column_content = sprintf('<a href="%s">%s</a>', get_edit_post_link($item['product_id']), Subscription_Order::get_item_name($item['order_id'], $item['product_id']));
...
//
//				$order = new WC_Order($item['order_id']);
//				$order_item = Subscription_Order::get_item_by_product_id($order, $item['product_id']);
...
//				$column_content = sprintf('<a href="%1$s">%2$s</a>', get_edit_post_link($item[$column_name]), sprintf(__('Order %s', 'plugin'), $order->get_order_number()));
...
//				{
//					$column_content = __('Never', 'plugin');
//				}
//				else if ($column_name == 'end_date' && $item[$column_name] == 0)
//				{
//					$column_content = __('Not yet ended', 'plugin');
//				}
//				else
//				{
...
//				$trial_expiration = Subscription_Manager::get_trial_expiration_date($item['subscription_key'], $item['user_id'], 'timestamp');
...
//						$last_payment = sprintf(__('%s ago', 'plugin'), human_time_diff($last_payment_timestamp, current_time('timestamp')));
//					else
...
//
//				$next_payment_timestamp = Subscription_Manager::get_next_payment_date($item['subscription_key'], $item['user_id'], 'timestamp');
//
...
//
//					if ($time_diff > 0 && $time_diff < 7 * 24 * 60 * 60)
//						$next_payment = sprintf(__('In %s', 'plugin'), human_time_diff(current_time('timestamp'), $next_payment_timestamp));
//					else
...
//
//					if (Subscription_Manager::can_subscription_be_changed_to('new-payment-date', $item['subscription_key'], $item['user_id']))
//					{
...
//						$column_content .= '<a href="#edit_timestamp" class="edit-timestamp" tabindex="4">' . __('Change', 'plugin') . '</a>';
//						$column_content .= '<div class="date-picker-div hide-if-js">';
//						$column_content .= Subscription_Manager::touch_time(array(
...
////						'<a href="%1$s">%2$d</a>', admin_url('edit.php?post_status=all&post_type=shop_order&_renewal_order_parent_id=' . absint($item['order_id'])), $count
////				);
//				break;
//		}
//
//		return $column_content;
	...
	public function get_columns()
	{
		...
		$columns = array(
			...
			'status' => __('Status', 'plugin'),
			'title' => __('Subscription', 'plugin'),
			'user' => __('User', 'plugin'),
			'order_id' => __('Order', 'plugin'),
			'start_date' => __('Start Date', 'plugin'),
			'expiry_date' => __('Expiration', 'plugin'),
			'end_date' => __('End Date', 'plugin'),
			'trial_expiry_date' => __('Trial End Date', 'plugin'),
			'last_payment_date' => __('Last Payment', 'plugin'),
			'next_payment_date' => __('Next Payment', 'plugin'),
			'renewal_order_count' => __('Renewals', 'plugin'),
		);
	...
	public function get_bulk_actions()
	{
		...
		if (isset($_REQUEST['status']) && $_REQUEST['status'] == 'trash')
			$actions = array('deleted' => __('Delete Permanently', 'plugin'));
		else if (!isset($_REQUEST['status']) || $_REQUEST['status'] != 'trash')
			$actions = array('trash' => __('Move to Trash', 'plugin'));
...
//		}
//		else if (isset($_GET['_customer_user']) || isset($_GET['_product_id']))
//		{ // Single subscription action
//			if (!wp_verify_nonce($_REQUEST['_wpnonce'], 'bulk-subscriptions'))
//				wp_die(__('Action failed. Invalid Nonce.', 'plugin'));
//
...
//
//				if (false === $user)
//					wp_die(__('Action failed. Invalid user ID.', 'plugin'));
//
...
//
//				if (false === $product)
//					wp_die(__('Action failed. Invalid product ID.', 'plugin'));
//
...
//				wp_die(__('Action failed. Invalid Nonce.', 'plugin'));
//
//			if (!Subscription_Manager::can_subscription_be_changed_to($_GET['new_status'], $_GET['subscription'], $_GET['user']))
//			{
//				$error_messages[] = sprintf(__('Error: Subscription status can not be changed to "%s".', 'plugin'), esc_html($_GET['new_status']));
//			}
//			else
//			{
...
//				{
//					case 'active' :
//						Subscription_Manager::reactivate_subscription($_GET['user'], $_GET['subscription']);
//						$messages[] = __('Subscription activated.', 'plugin');
//						break;
//					case 'on-hold' :
//						Subscription_Manager::put_subscription_on_hold($_GET['user'], $_GET['subscription']);
//						$messages[] = __('Subscription put on-hold.', 'plugin');
//						break;
//					case 'cancelled' :
//						Subscription_Manager::cancel_subscription($_GET['user'], $_GET['subscription']);
//						$messages[] = __('Subscription cancelled.', 'plugin');
//						break;
//					case 'trash' :
//						Subscription_Manager::trash_subscription($_GET['user'], $_GET['subscription']);
//						$messages[] = __('Subscription trashed.', 'plugin');
//						break;
//					case 'deleted' :
//						Subscription_Manager::delete_subscription($_GET['user'], $_GET['subscription']);
//						$messages[] = __('Subscription deleted.', 'plugin');
//						break;
//					default :
//						$error_messages[] = __('Error: Unknown subscription status.', 'plugin');
//						break;
//				}
//			}
//		}
//		else if (isset($_GET['subscription_keys']))
//		{ // Bulk actions
//			if (!wp_verify_nonce($_REQUEST['_wpnonce'], 'bulk-' . $this->_args['plural']))
//				wp_die(__('Bulk edit failed. Invalid Nonce.', 'plugin'));
//
...
//					{
//						if (!Subscription_Manager::can_subscription_be_changed_to($this->current_action(), $subscription_key, $user_id))
//						{
//							$error_count++;
//						}
//						else
...
//								Subscription_Manager::trash_subscription($user_id, $subscription_key);
//							else
//								Subscription_Manager::delete_subscription($user_id, $subscription_key);
//						}
//					}
//				}
//
...
//				{
//					if ('trash' == $this->current_action())
//						$messages[] = sprintf(_n('%d subscription moved to trash.', '%s subscriptions moved to trash.', $subscription_count, 'plugin'), $subscription_count);
//					else
//						$messages[] = sprintf(_n('%d subscription deleted.', '%s subscriptions deleted.', $subscription_count, 'plugin'), $subscription_count);
//				}
//
//				if ($error_count > 0)
//				{
//					if ('trash' == $this->current_action())
//						$error_messages[] = sprintf(_n('%d subscription could not be trashed - is it active or on-hold? Try cancelling it before trashing it.', '%s subscriptions could not be trashed - are they active or on-hold? Try cancelling them before trashing.', $error_count, 'plugin'), $error_count);
//					else
//						$error_messages[] = sprintf(_n('%d subscription could not deleted - is it active or on-hold? Try cancelling it before deleting.', '%s subscriptions could not deleted - are they active or on-hold? Try cancelling them before deleting.', $error_count, 'plugin'), $error_count);
//				}
//			}
//		}
//
...
//		exit;
	...
	public function messages()
	{
		...
		} elseif (isset($_REQUEST['s']))
		{
			...
			printf(__('Showing only subscriptions matching "%s"', 'plugin'), esc_html($_REQUEST['s']));
			echo '</p></div>';
		}
		...
		if (isset($_GET['_customer_user']) || isset($_GET['_product_id']))
		{
			...
			if (!empty($_GET['_customer_user']))
			{
				...
				if (false === $user)
					printf(__('Invalid user. ', 'plugin'), $user->display_name);
				else
					printf(__("Showing %s's subscriptions", 'plugin'), $user->display_name);
			}
			...
			if (!empty($_GET['_product_id']))
			{
				...
				if (false === $product)
					printf(__('Invalid product.', 'plugin'), $user->display_name);
				elseif (!empty($_GET['_customer_user']))
					printf(__(' for product #%s &ndash; %s%s%s', 'plugin'), $product_id, '<em>', $product->get_title(), '</em>');
				else
					printf(__('Showing subscriptions to product #%s &ndash; %s%s%s', 'plugin'), $product_id, '<em>', $product->get_title(), '</em>');
			}
	...
	function prepare_items()
	{
		...
		if (isset($_REQUEST['s']))
		{
			$subscriptions_grouped_by_user = Subscription_Manager::search_subscriptions($_REQUEST['s']);
		}
		elseif (isset($_GET['_customer_user']) || isset($_GET['_product_id']))
		{

			if (isset($_GET['_customer_user']) && !empty($_GET['_customer_user']))
				$subscriptions_grouped_by_user = array($_GET['_customer_user'] => Subscription_Manager::get_users_subscriptions($_GET['_customer_user']));
			else
				$subscriptions_grouped_by_user = Subscription_Manager::get_all_users_subscriptions();
	...
	function extra_tablenav($which)
	{
		...
		{
			?>
			<select id="dropdown_products_and_variations" name="_product_id" data-placeholder="<?php _e('Search for a product&hellip;', 'plugin'); ?>" style="width: 240px">
	...
	public function no_items()
	{
		...
		if (isset($_GET['_customer_user']) || !empty($_GET['_customer_user'])) :
			...
			<?php if (isset($_REQUEST['_product_id']) || isset($_REQUEST['_product_id'])) : ?>
				<p><?php printf(__('%s has not purchased %s%s%s (#%s).', 'plugin'), $user->display_name, '<em>', $product->get_title(), '</em>', intval($_GET['_product_id'])); ?></p>
			<?php else : ?>
				<p><?php printf(__('%s has not purchased any subscriptions.', 'plugin'), $user->display_name); ?></p>
			<?php endif; ?>
		<?php elseif (isset($_REQUEST['_product_id']) || isset($_REQUEST['_product_id'])) : ?>
			<p><?php printf(__('No one has purchased %s%s%s (#%s) yet.', 'plugin'), '<em>', $product->get_title(), '</em>', intval($_GET['_product_id'])); ?></p>
		<?php elseif (isset($_REQUEST['s'])) : ?>
			<p><?php printf(__('No subscriptions found to have a subscriber name, email, title or order ID matching "%s".', 'plugin'), '<em>' . esc_attr($_REQUEST['s']) . '</em>'); ?></p>
		<?php else : ?>
			<p><?php _e('Subscriptions will appear here for you to view and manage once purchased by a customer.', 'plugin'); ?></p>
			<p><?php printf(__('%sLearn more about managing subscriptions%s', 'plugin'), '<a href="http://docs.woothemes.com/document/store-manager-guide/#managingsubscriptions" target="_blank">', ' &raquo;</a>'); ?></p>
			<p><?php printf(__('%sAdd a subscription product%s', 'plugin'), '<a href="' . WC_Subscriptions_Admin::add_subscription_url() . '">', ' &raquo;</a>'); ?></p>
		<?php
		endif;
	}
	...
	function sort_subscriptions($a, $b)
	{
		...
		switch ($order_by)
		{
			...
			case 'end_date' :
				...
				break;
			case 'next_payment_date' :
				$next_payment_a = Subscription_Manager::get_next_payment_date($a['subscription_key'], $a['user_id'], 'mysql');
				$next_payment_b = Subscription_Manager::get_next_payment_date($b['subscription_key'], $b['user_id'], 'mysql');
			...
			case 'last_payment_date' :
				...
				break;
			case 'trial_expiry_date':
				$trial_expiration_a = Subscription_Manager::get_trial_expiration_date($a['subscription_key'], $a['user_id'], 'mysql');
				$trial_expiration_b = Subscription_Manager::get_trial_expiration_date($b['subscription_key'], $b['user_id'], 'mysql');
