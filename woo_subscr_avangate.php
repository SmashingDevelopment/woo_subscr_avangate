...
defined('AVANGATE_PATH') || define('AVANGATE_PATH', plugin_dir_path( __FILE__ ));

spl_autoload_register('avangate_subscription_loader');
function avangate_subscription_loader($class) {
	if (!class_exists($class))
	{
		$theme_class_path = AVANGATE_PATH . DIRECTORY_SEPARATOR . 'classes/' . str_replace('_', DIRECTORY_SEPARATOR, $class) . '.php';
...
//require_once( 'classes/gateways/gateway-avangate.php' );
...
class WC_Subscriptions {
	...
	public static function init() {
		...
		//@custom
		add_filter( 'woocommerce_payment_gateways', __CLASS__ . '::add_avangate_gateway_class' );
	}
	...
	public static function get_my_subscriptions_template() {
		...
		foreach ( $subscriptions as $subscription_key => $subscription_details ) {
			...
			$last_order_id = end( WC_Subscriptions_Renewal_Order::get_renewal_orders( $subscription_details['order_id'], 'ID' ) );
	...
	public static function add_to_cart_redirect( $url ) {
		...
		if ( is_numeric( $_REQUEST['add-to-cart'] ) && WC_Subscriptions_Product::is_subscription( (int) $_REQUEST['add-to-cart'] ) ) {
			/**
			 * @todo  add avangate url with product id in it
			 */
...
<?php
	...
	public static function add_avangate_gateway_class ( $methods ) {
...
//		var_dump($instance->getClient()->getCustomerSubscriptions($sessionID, '', ''));
//	}
//	catch(SoapFault $e) {
//		var_dump($e->getMessage());
//		die();
//	}
////	var_dump(Avangate_Client::getInstance()->getErrorMessage());
//	die();
//	global $wp_filesystem;
//	$wp_filesystem->put_contents(
//	  '/web/templateinvaders/www/wp-content/log.txt',
//		var_export($client, 1),
